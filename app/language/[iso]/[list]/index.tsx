...
export default function ListPage() {
  ...
  const handleExport = async () => {
    try {
      ...
      if (Platform.OS === "web") {
        ...
        const fileName = `QuickLearner_${exportedList.name
          .replace(/[^a-z0-9]/gi, "_")
          .toLowerCase()}.json`;
  ...
  const handlePracticeSentences = () => {
    if (state.currentLanguage && state.currentList) {
      const wordUuids = selectedWords.map((word) => word.uuid);
      goToSentences(wordUuids);
    } else {
  ...
  if (isLoading && (!words || words.length === 0)) {
    return (
      <SafeAreaWrapper backgroundColor={colors.background}>
        <UnifiedHeader title={state.currentList.name} />
        <View
          style={[
            styles.container,
            {
  ...
  return (
    <SafeAreaWrapper backgroundColor={colors.background}>
      <UnifiedDialog
        ...
        actions={
          <UnifiedButton onPress={() => setSelectedWord(null)}>
            {i18n.t("close")}
          </UnifiedButton>
        }
      >
      ...
      <UnifiedDialog
        ...
        actions={
          <UnifiedButton onPress={() => setShowSortMenu(false)}>
            {i18n.t("cancel")}
          </UnifiedButton>
        }
      >
      ...
      <UnifiedHeader
        ...
        actions={
          selectMode ? (
            <View style={{ flexDirection: "row", gap: 8, height: 40 }}>
              <Button
                mode="contained"
            ...
            <View style={{ flexDirection: "row", gap: 8, height: 40 }}>
              <IconButton icon="export" size={24} onPress={handleExport} />
              <IconButton icon="import" size={24} onPress={handleImport} />
              <IconButton
                icon="sort"
                size={24}
                style={{ marginRight: -6 }}
              ...
              >
                <View
                  style={[
                    entryStyles.card,
                    { backgroundColor: colors.background },
                    selectedWords.find((w) => w.uuid === word.uuid)
                  ...
                  >
                    <View style={[entryStyles.cardRowContent]}>
                      <View style={entryStyles.cardColumnContent}>
                        <Text style={[typography.bold, { color: colors.text }]}>
                          {word.translation}
                        </Text>
                        <Text
                          style={[
                            typography.small_body,
                            { color: colors.muted },
                          ]}
                        >
                          {word.word}
                        ...
                        {word.example && (
                          <Text
                            style={[typography.italic, { color: colors.muted }]}
                          >
                            {word.example}
                          </Text>
                        )}
